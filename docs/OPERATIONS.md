...
python monitor_tui.py         # full-screen TUI version (panels, colors; q quit / p pause / r refresh)
...
| 9:15–14:30 | Checks all 30 stocks every bar (5 min default; 1 min with `TRADE_INTERVAL=1m`); buys breakouts, manages open trades. Pauses fresh entries 11:45–13:30 (the midday lull) and after 6 entries/day | Nothing. Seriously — don't touch it. Watching every tick teaches anxiety, not skill. |
...
- `python -m pytest tests/ -q` — 95 self-tests. Run after any change you make to the code.
