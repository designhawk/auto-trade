...
| `RSI_MIN` / `RSI_MAX` | 30 / 75 | RSI entry gate. `RSI_MAX=75` refuses exhausted moves (kept many strong names out: SANSERA 77–81, ACMESOLAR 78–88); raise to 85 in paper mode to collect data on those. |
...
| Entry window: 9:30–14:45 (optional midday pause, **off by default**) | fixed times | The open's first 15 min is false-breakout territory and 14:45-onward leaves no time to recover. A **lunch-lull pause is available but disabled**: 11:45–13:30 is the highest-loss window in Indian intraday studies, but paper trading wants the data. Enable with `ENTRY_PAUSE_START=11:45`, `ENTRY_PAUSE_END=13:30`. |
